...
#first I make a new data frame with just protein coding data, removing rows with missing data
...
summary <- locale %>% subset(!is.na(mRNA.stabilities)) %>% summarySE(measurevar = "mRNA.stabilities", groupvars = c("localisation", "essential"))
summary
...
#N/a values are automatically removed by this function so subset did not need to be used
...
#
#As explained "79 observations deleted due to missingness" automatically
